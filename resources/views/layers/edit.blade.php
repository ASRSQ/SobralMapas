            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('layers.update', $layer->id) }}">
                            ...
                            <div class="col-md-6">
                                <select id="category_id" class="form-control @error('category_id') is-invalid @enderror" name="category_id" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ $layer->category_id == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <!-- Adicionar seleção de subcategoria -->
                        <div class="form-group row mt-2">
                            ...
                            <div class="col-md-6">
                                <select id="subcategory_id" class="form-control @error('subcategory_id') is-invalid @enderror" name="subcategory_id" required>
                                    ...
                                    @foreach($subcategories as $subcategory)
                                        <option value="{{ $subcategory->id }}" {{ $layer->subcategory_id == $subcategory->id ? 'selected' : '' }}>
                                            {{ $subcategory->name }}
                                        </option>
                                    @endforeach
                                </select>
